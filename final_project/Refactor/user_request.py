import pygame
from tower import Tower, Vacancy
    ...
    def update(self, user_request: str, model):
        ...
        if user_request in ['damage', 'range', 'cd_time']:
            ...
            if model.selected_tower.level[upgrade_name] < 5:
                if model.money > model.selected_tower.get_upgrade_cost(upgrade_name):
                    model.money -= model.selected_tower.get_upgrade_cost(upgrade_name)
                    model.selected_tower.level[upgrade_name]+=1
